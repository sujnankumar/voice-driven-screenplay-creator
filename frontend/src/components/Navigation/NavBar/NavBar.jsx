...
const ProfileDropDown = (props) => {
    ...
    return (
        <div className={`relative ${props}`}>
            ...
            <ul
                className={`bg-darkGray text-white top-12 right-0 hidden lg:inline-block mt-5 space-y-5 lg:absolute lg:text-sm lg:w-52 lg:shadow-lg lg:space-y-0 lg:mt-0 ${
                    state ? "" : "lg:hidden"
                }`}
            >
                {navigation.map((item, idx) => (
                    <li key={idx + 238}>
                        <a
                            key={idx}
                            className="block text-inherit lg:hover:bg-darkGray lg:p-2.5"
                            href={item.path}
                        >
                            {item.title}
                        </a>
...
const NavBar = () => {
    ...
    return (
        <>
            <nav
                className={`relative shadow shadow-black z-30 lg:p-1 bg-[#18181B] w-full lg:fixed lg:border-none ${
